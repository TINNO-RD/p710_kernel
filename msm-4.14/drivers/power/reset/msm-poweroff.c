 ...
 */
#ifdef JOURNEY_FEATURE_CUSTOMER_VERSION
static int download_mode = 0; // customer version disable it .
#else
...
#if defined(JOURNEY_DEBUG_ENHANCED) && !defined(JOURNEY_FEATURE_CUSTOMER_VERSION)
bool journey_debug_reboot_panic = false;
static int __init journey_debug_reboot_panic_setup(char *str) {
	pr_info("%s",__FUNCTION__);
	journey_debug_reboot_panic = true;
    return 1;
}
__setup("journey.debug.reboot.panic", journey_debug_reboot_panic_setup);
#endif
#if defined(JOURNEY_DEBUG_ENHANCED) && !defined(JOURNEY_FEATURE_CUSTOMER_VERSION)
bool journey_debug_reboot_warm = false;
static int __init journey_debug_reboot_warm_setup(char *str) {
	pr_info("%s",__FUNCTION__);
	journey_debug_reboot_warm = true;
    return 1;
}
__setup("journey.debug.reboot.warm", journey_debug_reboot_warm_setup);
#endif
...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (qpnp_pon_check_hard_reset_stored()) {
		...
		if (get_dload_mode() || in_panic ||    //odm add panic for pstore
			((cmd != NULL && cmd[0] != '\0') &&
			!strcmp(cmd, "edl")))
			need_warm_reset = true;
	} else {
		need_warm_reset = (get_dload_mode() || in_panic ||    //odm add panic for pstore
				(cmd != NULL && cmd[0] != '\0'));
	}
#if defined(JOURNEY_DEBUG_ENHANCED)
	pr_info("msm_restart_prepare: cmd '%s' need_warm_reset='%d' get_dload_mode()='%d' qpnp_pon_check_hard_reset_stored()='%d'\n",
		cmd,need_warm_reset,get_dload_mode(),qpnp_pon_check_hard_reset_stored());
	pr_info("msm_restart_prepare: in_panic='%d' restart_mode='%d'\n",
		in_panic,restart_mode);
#endif
#if defined(JOURNEY_DEBUG_ENHANCED) && !defined(JOURNEY_FEATURE_CUSTOMER_VERSION)
    if(journey_debug_reboot_warm) {
		pr_info("Forcing a warm reset of the system because bootloader command\n");
        force_warm_reboot = true;
    } else {
#ifdef JOURNEY_FEATURE_FORCE_WARM_RESET
        pr_info("Forcing a warm reset of the system because debug build\n");
        force_warm_reboot = true;
#endif
    }
#endif
...
#if defined(JOURNEY_DEBUG_ENHANCED) && !defined(JOURNEY_FEATURE_CUSTOMER_VERSION)
    BUG_ON(journey_debug_reboot_panic);
#endif
