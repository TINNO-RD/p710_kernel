...
#define UPDATE_ADJ_MSOC_DELAY_CHG_MS		10000
#define UPDATE_ADJ_MSOC_DELAY_DISCHG_MS		60000
#endif
...
//UI SOC optimize 100%~80% -5%; 20%~0% -5%; 80%~20% +10% PHBAAE-1625 by pony date20200721 start
#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
struct uisoc_map_soc {
	int before_uisoc;
	int after_uisoc;
};

static struct uisoc_map_soc vals[] = {
//100%~80% -5%; 20%~0% -5%; 80%~20% +10%
{0,    0},
{1,    1},
{2,    2},
{3,    3},
...
int get_ui_soc_index(int val)
{
	...
	for (i = 0; i < sizeof(vals)/sizeof(vals[0]); i++) {
		if (soc < (vals[i].before_uisoc)) {
			break;
		}
	}

	return index_bound(i);
}

int get_ui_soc(int soc)
{
	int index = get_ui_soc_index(soc);

	printk("[get_ui_soc] index = %d, befor_soc = %d, ui_soc = %d\n", index, soc, vals[index].after_uisoc);

	return vals[index].after_uisoc;
}

static enum alarmtimer_restart
	qpnp_msoc_timer_adjust_msoc(struct alarm *alarm, ktime_t now)
{
	struct qpnp_qg *chip = container_of(alarm,
				struct qpnp_qg, alarm_timer_adjust_msoc);

	/* timer callback runs in atomic context, cannot use voter */
	pm_stay_awake(chip->dev);
	schedule_work(&chip->update_adj_msoc_work);

	return ALARMTIMER_NORESTART;
}

static void update_adj_msoc(struct qpnp_qg *chip)
{
	int adj_soc;
	struct timespec ts;
	static unsigned long last_sec = 0;
	
	adj_soc = get_ui_soc(chip->msoc);
	chip->adj_this_msoc = adj_soc;
	
	qg_dbg(chip, QG_DEBUG_SOC, "msoc = %d,adj_soc = %d,last_soc=%d,charge_status=%d\n",
		chip->msoc,adj_soc,chip->adj_last_msoc,chip->charge_status);
	alarm_cancel(&chip->alarm_timer_adjust_msoc);
	
	get_monotonic_boottime(&ts);
	if(abs(adj_soc - chip->adj_last_msoc) >= 2){
		if(chip->charge_status == POWER_SUPPLY_STATUS_CHARGING){
			if(adj_soc > chip->adj_last_msoc){
				chip->adj_msoc += chip->dt.delta_soc;
				alarm_start_relative(&chip->alarm_timer_adjust_msoc,
						ms_to_ktime(UPDATE_ADJ_MSOC_DELAY_CHG_MS));
			}
			if(adj_soc < chip->adj_last_msoc){
				chip->adj_msoc -= chip->dt.delta_soc;
				alarm_start_relative(&chip->alarm_timer_adjust_msoc,
						ms_to_ktime(UPDATE_ADJ_MSOC_DELAY_DISCHG_MS));
			}
		}
...
//		alarm_cancel(&chip->alarm_timer_adjust_msoc);
		chip->adj_msoc = adj_soc;		
	}

	if (chip->adj_msoc >= 100) {
		chip->adj_msoc = 100;
	} else if (chip->adj_msoc < 0) {
		chip->adj_msoc = 0;
	}
	
	chip->adj_last_msoc = chip->adj_msoc;

	qg_dbg(chip, QG_DEBUG_SOC, "ccc ts = %ld last_sec = %ld,msoc = %d,last_soc = %d,adj_soc = %d\n",
		ts.tv_sec,last_sec,chip->msoc,chip->adj_last_msoc,chip->adj_msoc);

	last_sec = ts.tv_sec;
}
#endif
//UI SOC optimize 100%~80% -10%; 25%~0% -10%; 80%~25% +20% PHBAAE-1625 by pony date20200721 end
...
static void update_msoc(struct qpnp_qg *chip)
{
	...
	//UI SOC optimize 100%~80% -10%; 25%~0% -10%; 80%~25% +20% PHBAAE-1625 by pony date20200721
#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
	update_adj_msoc(chip);
#endif
...
#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
static void update_adj_msoc_work(struct work_struct *work)
{
	struct qpnp_qg *chip = container_of(work,
			struct qpnp_qg, update_adj_msoc_work);

	mutex_lock(&chip->soc_lock);

	if((chip->adj_msoc >= 0) && (chip->adj_msoc < 100)){	
		if(chip->charge_status == POWER_SUPPLY_STATUS_CHARGING){
			if(chip->adj_this_msoc > chip->adj_last_msoc)
				chip->adj_msoc += chip->dt.delta_soc;
			if(chip->adj_this_msoc < chip->adj_last_msoc)
				chip->adj_msoc -= chip->dt.delta_soc;			
		}
	...
	chip->adj_last_msoc = chip->adj_msoc;	
	qg_dbg(chip, QG_DEBUG_SOC,
		"update_adj_msoc_work: adj_msoc=%d adj_last_msoc=%d,adj_this_msoc=%d\n",
...
int qg_soc_init(struct qpnp_qg *chip)
{
	...
	#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
	if (alarmtimer_get_rtcdev()) {
		alarm_init(&chip->alarm_timer_adjust_msoc, ALARM_BOOTTIME,
			qpnp_msoc_timer_adjust_msoc);
	} else {
		pr_err("Failed to get soc alarm-timer\n");
		return -EINVAL;
	}

	INIT_WORK(&chip->update_adj_msoc_work, update_adj_msoc_work);
	#endif
