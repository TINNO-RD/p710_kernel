...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	#ifdef CONFIG_SCC_SUPPORT	
	int byte_len_speed;				//by pony.ma for scc date20200618
	#endif
	...
	chg->dcp_icl_en= of_property_read_bool(node,
				"qcom,dcp-icl-en");

	rc = of_property_read_u32(node,
				"qcom,dcp-icl-ua-1", &chg->dcp_icl_ua_1);
	if (rc < 0)
		chg->dcp_icl_ua_1 = -EINVAL;
	...
	pr_err("dcp_icl_en=%d,dcp_icl_ua_1=%d,float_icl_ua=%d\n",chg->dcp_icl_en,chg->dcp_icl_ua_1,chg->float_icl_ua);
	//date20200429-01 end
	...
	//for scc by pony start date20200618
	#ifdef CONFIG_SCC_SUPPORT
	if (of_find_property(node, "qcom,speed-current", &byte_len_speed)) {
		chg->speed_mitigation = devm_kzalloc(chg->dev, byte_len_speed,
			GFP_KERNEL);

		if (chg->speed_mitigation == NULL)
			return -ENOMEM;

		chg->speed_levels = byte_len_speed / sizeof(u32);
		rc = of_property_read_u32_array(node,
				"qcom,speed-current",
				chg->speed_mitigation,
				chg->speed_levels);
		if (rc < 0) {
			dev_err(chg->dev,
				"Couldn't read speed limits rc = %d\n", rc);
			return rc;
		}
	}
	
	speed_levels = chg->speed_levels;
	scc_create_file();
	#endif
	//for scc by pony end
		...
		if (((chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_DEFAULT) ||
		   (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB))
			...
			if(last_speed != target_speed){
				...
				rc = smblib_set_prop_speed_level(chg, target_speed);			
			}
			#endif
			//for scc by pony end
		}
...
static enum power_supply_property smb5_batt_props[] = {
	POWER_SUPPLY_PROP_INPUT_SUSPEND,
	POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED,          //add charging enable node by pony.ma date20200415
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		...
		break;
	//add charging enable node by pony.ma date20200415
		...
		enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_STEP_CHARGING_ENABLED:
	case POWER_SUPPLY_PROP_DIE_HEALTH:
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:    //add charging enable node by pony.ma date20200415
		return 1;
	default:
		break;
	}
