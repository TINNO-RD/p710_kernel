...
//<REQ><PHBAAE-1026><poweroff,when low battery power>pony.ma-20200529--start
#ifdef CONFIG_LOWBATT_PWROFF
extern char *saved_command_line;
#define CHARGER_MODE_BOOT   "androidboot.mode=charger"
#endif
//<REQ><PHBAAE-1026><poweroff,when low battery power>pony.ma-20200529--end

#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
#define UPDATE_ADJ_MSOC_FULL_DELAY_CHG_MS		10000
#endif

static int qg_debug_mask = QG_DEBUG_PROFILE | QG_DEBUG_IRQ | QG_DEBUG_CUSTOM | QG_DEBUG_PON | QG_DEBUG_DEVICE | QG_DEBUG_SOC;
//static int qg_debug_mask = 0x1CFF;
module_param_named(
...
//add for TinnoProductInfo by pony date20200628
#ifdef CONFIG_TINNO_PRODUCT_INFO
#include <dev_info.h>
int bat_info;
int get_battery_vol_info(char *buf, void *arg0) {
	pr_err("bat_info=%d\n", bat_info);
	return sprintf(buf, "%d mv", bat_info);
}
#endif

// DATE20200326, QG trace information, DATE20200326-01 START
static int qg_get_battery_capacity(struct qpnp_qg *chip, int *soc);
static void qg_status_trace(struct qpnp_qg *chip)
{
	...
	struct power_supply *usb_psy1;
	int usb_v_now,usb_i_now,rc;

	usb_psy1 = power_supply_get_by_name("usb");
	if (usb_psy1) {
		rc = power_supply_get_property(usb_psy1,
				POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
		if (rc < 0)
			pr_err("Couldn't read USB usb_v_now, rc=%d\n", rc);
		else
			usb_v_now = pval.intval;
		rc = power_supply_get_property(usb_psy1,
				POWER_SUPPLY_PROP_INPUT_CURRENT_NOW, &pval);
		if (rc < 0)
			pr_err("Couldn't read USB usb_v_now, rc=%d\n", rc);
		else
			usb_i_now = pval.intval;
	}

	qg_get_battery_capacity(chip, &curr_soc);
	qg_get_battery_voltage(chip, &vbat);
	qg_get_battery_current(chip, &ibat);

	//add for TinnoProductInfo by pony date20200628
	#ifdef CONFIG_TINNO_PRODUCT_INFO
	{
	 bat_info = chip->sdam_data[SDAM_OCV_UV]/1000;
	...
	if (curr_soc != last_soc)
	{
		...
		qg_dbg(chip, QG_DEBUG_CUSTOM, "QG: BAT T[CHG,UI_SOC,OCV,VC,I,USB_V,USB_I,T]=%d[%d,%d,%d,%d,%d,%d,%d,%d],%lu,%lu\n",
			count,
			(is_usb_present(chip) ? 1 : 0), 		// plug/unplug
			curr_soc,								// %
			chip->sdam_data[SDAM_OCV_UV]/1000,		// mV
			vbat/1000,								// mV
			ibat/1000,								// mA
			usb_v_now/1000,
			usb_i_now/1000,
			chip->sdam_data[SDAM_TEMP],				// 0.1.C
	...
	{
		qg_dbg(chip, QG_DEBUG_CUSTOM, "QG: BAT V[CHG,UI_SOC,OCV,VC,I,USB_V,USB_I,T]bat_soc,cc_soc,full,done=%d[%d,%d,%d,%d,%d,%d,%d,%d][%d,%d,%d,%d]\n",
			count,
			(is_usb_present(chip) ? 1 : 0), 		// plug/unplug
			curr_soc,								// %
			chip->sdam_data[SDAM_OCV_UV]/1000,		// mV
...
static void process_udata_work(struct work_struct *work)
{
	...
	qg_dbg(chip, QG_DEBUG_SOC, "udata update: batt_soc=%d cc_soc=%d full_soc=%d qg_esr=%d\n",
		(chip->batt_soc != INT_MIN) ? chip->batt_soc : -EINVAL,
		(chip->cc_soc != INT_MIN) ? chip->cc_soc : -EINVAL,
		chip->full_soc, chip->esr_last);

	// QG trace information, DATE20200326
	qg_status_trace(chip);
...
static irqreturn_t qg_fifo_update_done_handler(int irq, void *data)
{
	...
	qg_dbg(chip, QG_DEBUG_FIFO, "IRQ triggered\n");
...
static bool check_poweroff_charger(void)
{
	...
	pr_err("check_poweroff_charger:chg_boot_flag=%d\n",chg_boot_flag);
...
static int qg_reset(struct qpnp_qg *chip)
{
	...
	qg_dbg(chip, QG_DEBUG_IRQ, "QG RESET triggered\n");
	...
	qg_dbg(chip, QG_DEBUG_IRQ, "VBAT=%duV SOC=%d\n", rc, soc);
	...
	if (count == MAX_QG_OK_RETRIES) {
		qg_dbg(chip, QG_DEBUG_IRQ, "QG_OK not set\n");
		goto done;
	}
	...
	qg_dbg(chip, QG_DEBUG_IRQ, "S7_OCV = %duV\n", ocv_uv);
...
//UI SOC optimize 100%~80% -10%; 25%~0% -10%; 80%~25% +20% PHBAAE-1625 by pony date20200721 start
#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
static enum alarmtimer_restart
	qpnp_msoc_timer_adjust_msoc_full(struct alarm *alarm, ktime_t now)
{
	struct qpnp_qg *chip = container_of(alarm,
				struct qpnp_qg, alarm_timer_adjust_msoc_full);

	/* timer callback runs in atomic context, cannot use voter */
	pm_stay_awake(chip->dev);
	schedule_work(&chip->update_adj_msoc_full_work);

	return ALARMTIMER_NORESTART;
}

static void update_adj_msoc_full_work(struct work_struct *work)
{
	struct qpnp_qg *chip = container_of(work,
			struct qpnp_qg, update_adj_msoc_full_work);

	mutex_lock(&chip->soc_lock);

	if(chip->charge_done && (chip->adj_this_msoc > chip->adj_last_msoc) && (chip->adj_msoc < 99))	
			chip->adj_msoc += chip->dt.delta_soc;
	
	chip->adj_last_msoc = chip->adj_msoc;	
	
	qg_dbg(chip, QG_DEBUG_SOC,
		"update_adj_msoc_full_work: adj_msoc=%d adj_last_msoc=%d,adj_this_msoc=%d\n",
			chip->adj_msoc, chip->adj_last_msoc,chip->adj_this_msoc);
...
static int qg_charge_full_update(struct qpnp_qg *chip)
{
	...
	if (chip->charge_done && !chip->charge_full) {
		#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
		if (chip->adj_msoc >= 99 && health == POWER_SUPPLY_HEALTH_GOOD) {
			alarm_cancel(&chip->alarm_timer_adjust_msoc_full);
		#else
		...
		#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
		else if (chip->adj_msoc < 99) {
			/* adjust msoc to full when adj_msoc very low msoc */
			alarm_start_relative(&chip->alarm_timer_adjust_msoc_full,
					ms_to_ktime(UPDATE_ADJ_MSOC_FULL_DELAY_CHG_MS));
			qg_dbg(chip, QG_DEBUG_STATUS, "adjust_msoc msoc=%d,adj_msoc=%d\n",
					chip->msoc,chip->adj_msoc);
		}
		#endif
...
use_pon_ocv:
	...
	#if defined(CONFIG_TINNO_BATTERY_UI_SOC_OPTIMIZE)
	chip->adj_msoc = chip->adj_last_msoc = get_ui_soc(soc);
	pr_info("using %s @ PON ocv_uv=%duV soc=%d,adj_last_msoc=%d,adj_msoc=%d\n",
			ocv_type, ocv_uv, chip->msoc,chip->adj_last_msoc,chip->adj_msoc);
	#endif
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	qg_dbg(chip, QG_DEBUG_PON, "DT: vbatt_empty_mv=%dmV vbatt_low_mv=%dmV delta_soc=%d ext-sns=%d vbatt_cutoff_mv=%d\n",
			chip->dt.vbatt_empty_mv, chip->dt.vbatt_low_mv,
			chip->dt.delta_soc, chip->dt.qg_ext_sense,chip->dt.vbatt_cutoff_mv);
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	if (alarmtimer_get_rtcdev()) {
		alarm_init(&chip->alarm_timer_adjust_msoc_full, ALARM_BOOTTIME,
			qpnp_msoc_timer_adjust_msoc_full);
	} else {
		pr_err("Failed to get soc alarm_timer_adjust_msoc_full\n");
		return -EINVAL;
	}

	INIT_WORK(&chip->update_adj_msoc_full_work, update_adj_msoc_full_work);
	#endif
