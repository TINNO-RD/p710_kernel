...
//for scc by pony start date20200618
#ifdef CONFIG_SCC_SUPPORT
int smblib_set_prop_speed_level(struct smb_charger *chg,int scc_speed)
{
	if (scc_speed < 0)
		return -EINVAL;

	if (chg->speed_levels <= 0)
		return -EINVAL;

	if (scc_speed > chg->speed_levels)
		return -EINVAL;

	chg->speed_level = scc_speed;
	
	smblib_err(chg, "chg->speed_level=%d,chg->speed_levels=%d\n",chg->speed_level,chg->speed_levels);

	if (chg->speed_level == chg->speed_levels)
		return vote(chg->chg_disable_votable,
			SPEED_DAEMON_VOTER, true, 0);

	vote(chg->chg_disable_votable, SPEED_DAEMON_VOTER, false, 0);
	if (chg->speed_level == 0)
		return vote(chg->fcc_votable, SPEED_DAEMON_VOTER, false, 0);

	vote(chg->fcc_votable, SPEED_DAEMON_VOTER, true,
			chg->speed_mitigation[chg->speed_level]);
	return 0;
}
#endif
//for scc by pony.ma end
				...
				const union power_supply_propval *val)
{
	...
	smblib_err(chg, "chg->system_temp_level=%d,chg->thermal_levels=%d,thermal_current=%d\n",
		chg->system_temp_level,chg->thermal_levels,chg->thermal_mitigation[chg->system_temp_level]);

	//usb_icl for thermal by pony date20200925 start
	if(chg->system_temp_level == 8){
		vote(chg->usb_icl_votable, THERMAL_ICL_MAX_VOTER, true,
			chg->thermal_mitigation[chg->system_temp_level]);
		return 0;
	}
	else{
		vote(chg->usb_icl_votable, THERMAL_ICL_MAX_VOTER, true, chg->dcp_icl_ua_1);
	}
	//usb_icl for thermal by pony date20200925 end
				 ...
				 union power_supply_propval *val)
{
	...
	//for debug by pony date20200905 start
	union power_supply_propval pval = {0, };
	struct power_supply *usb_psy1;
	int usb_v_now,usb_i_now;
	...
	//for debug by pony date20200905 end
	...
	pr_err("pmi632_chg_temp = %d,USB_V=%d,USB_I=%d\n", val->intval,usb_v_now/1000,usb_i_now/1000);
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	smblib_dbg(chg, PR_MISC, "TYPE_C_STATUS_1 = 0x%02x\n", stat);

	switch (stat & DETECTED_SRC_TYPE_MASK) {
	case SNK_RP_STD_BIT:
	case SNK_DAM_500MA_BIT:	// Modify by Tinno
		return POWER_SUPPLY_TYPEC_SOURCE_DEFAULT;
	case SNK_RP_1P5_BIT:
	case SNK_DAM_1500MA_BIT:	// Modify by Tinno
		return POWER_SUPPLY_TYPEC_SOURCE_MEDIUM;
	case SNK_RP_3P0_BIT:
	case SNK_DAM_3000MA_BIT:	// Modify by Tinno
...
static int smblib_get_prop_typec_mode(struct smb_charger *chg)
{
	...
	smblib_dbg(chg, PR_MISC, "TYPE_C_MISC_STATUS_REG = 0x%02x\n", stat);
...
static int get_rp_based_dcp_current(struct smb_charger *chg, int typec_mode)
{
	...
	smblib_dbg(chg, PR_MISC, "typec_mode = %d\n",typec_mode);
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			...
			if (chg->connector_type ==
				...
				//for dcp icl config by pony date20200429-01 start
				if(chg->dcp_icl_en)
					rc = vote(chg->usb_icl_votable,
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB_DCP:
		...
		//for dcp icl config by pony date20200429-01 start
		if(chg->dcp_icl_en)
			vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, chg->dcp_icl_ua_1);
		else
		//date20200429-01 end
			vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, rp_ua);
		break;
