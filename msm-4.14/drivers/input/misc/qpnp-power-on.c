...
extern bool journey_debug_reboot_warm;
#endif
...
#if defined(JOURNEY_FEATURE_MANUAL_RAMDUMP) && !defined(JOURNEY_FEATURE_CUSTOMER_VERSION)
	if(!journey_debug_reboot_warm) {
	    cfg->s2_type = PON_POWER_OFF_TYPE_WARM_RESET;
	    dev_info(pon->dev,"s2_type force change to %d becase JOURNEY_FEATURE_MANUAL_RAMDUMP",cfg->s2_type);
	}
#endif
