...
static int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
	...
	for(i=0; i<TS_BIN_VERSION_LEN; i++) {
		ts_info("config version:%02X", cfg_bin->head.bin_version[i]);
	}
...
static int goodix_cfg_bin_proc(struct goodix_ts_core *core_data)
{
	...
	if (!r) {
		...
		ts_dev->chip_version.cfg_version = cfg_bin->head.bin_version[0];
	} else {
...
#ifdef JOURNEY_FEATURE_FACTORY_VERSION_TPLCD_TEST 
static int goodix_later_init2_thread(void *data)
{
	int ret;
	struct goodix_ts_core *ts_core = data;
	struct goodix_ts_device *ts_dev;
...
//					  UPDATE_MODE_FORCE|
	...
	ret = goodix_ts_stage3_init(ts_core);
	if (!ret) {
		ts_info("stage3 init success");
		return ret;
	}
	ts_err("stage3 init failed, %d", ret);
	return ret;
}
int goodix_later_init_func(struct goodix_ts_core *ts_core)
{
	struct task_struct *init_thrd;
	ts_info("%s", __func__);

	/* create and run update thread */
	init_thrd = kthread_run(goodix_later_init2_thread,
				ts_core, "goodix_init2_thread");
