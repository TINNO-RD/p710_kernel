 ...
 */
#ifdef JOURNEY_FEATURE_FACTORY_VERSION_TPLCD_TEST 
int goodix_ts_stage3_init(struct goodix_ts_core *core_data)
{
	int r;
	...
	if (r < 0) {
		...
		ts_info("succeed send normal config");

	r = ts_dev->hw_ops->read_version(ts_dev, &ts_dev->chip_version);
	if (r < 0)
		ts_info("failed read fw version info[ignore]");
	else
		ts_info("succeed read fw version info");

	return 0;
}
extern void goodix_later_init_func(void *data); 
#endif
...
#ifdef JOURNEY_FEATURE_FACTORY_VERSION_TPLCD_TEST 
{
	...
	if (first_in)
		...
		goodix_later_init_func(core_data);
}
#endif
...
#ifdef GOODIX_SPEED_LIGHT_UP
void goodix_resume_work_func(struct work_struct *work)
{
	struct goodix_ts_core *core_data;
	ts_info("%s start", __func__);
	core_data = container_of(work, struct goodix_ts_core, work);
	...
	unsigned long event, void *data)
{
	...
	ts_err("event = %d, blank = %d", event, blank);

	if (event != MSM_DRM_EVENT_BLANK)
		return 0;

	if (evdata->data && core_data) {
		switch (blank) {
...
#ifdef GOODIX_SPEED_LIGHT_UP
			ts_info("resume touch when idle");
			queue_work(core_data->goodix_wq, &core_data->work);
#else
			goodix_ts_resume(core_data);
#endif
			break;
...
#ifdef CONFIG_DRM_MSM
	if (msm_drm_register_client(&core_data->fb_notifier) < 0)
...
#ifdef GOODIX_SPEED_LIGHT_UP
	core_data->goodix_wq = create_singlethread_workqueue("goodix_resume_wq");
	INIT_WORK(&core_data->work, goodix_resume_work_func);
#endif
...
#ifdef GOODIX_SPEED_LIGHT_UP
	destroy_workqueue(core_data->goodix_wq);
#endif
