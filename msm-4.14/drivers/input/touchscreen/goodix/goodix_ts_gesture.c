...
#ifdef CONFIG_TINNO_TP_GESTURE
#include <tgesture/tgesture_global.h>
#endif
	...
	struct goodix_ext_module *module)
{
	...
	ts_debug("data[0-3]0x%x, 0x%x, 0x%x, 0x%x", temp_data[0], temp_data[1],
...
#ifdef CONFIG_TINNO_TP_GESTURE
		if (temp_data[2] == 'F')
			TGesture_dev_report('F');
		else if ((temp_data[2] == 'L')||(temp_data[2] == 0xFF))
			TGesture_dev_report('U');
#else
...
static int gsx_gesture_after_resume(struct goodix_ts_core *core_data,
	struct goodix_ext_module *module)
{
