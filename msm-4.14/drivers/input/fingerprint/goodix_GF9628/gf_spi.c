...
static void nav_event_input(struct gf_dev *gf_dev, gf_nav_event_t nav_event)
{
	...
	case GF_NAV_FINGER_DOWN:
		pr_err("%s nav finger down\n", __func__);
		break;

	case GF_NAV_FINGER_UP:
		pr_err("%s nav finger up\n", __func__);
		break;
	...
	case GF_NAV_LONG_PRESS:
		...
		pr_err("%s nav long press\n", __func__);
		break;

	case GF_NAV_DOUBLE_CLICK:
		nav_input = GF_NAV_INPUT_DOUBLE_CLICK;
		pr_err("%s nav double click\n", __func__);
		break;
...
static int irq_setup(struct gf_dev *gf_dev)
{
	...
	pr_err("gf success to request IRQ:%d\n", gf_dev->irq);
...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	pr_err("%s: received key event[%d], key=%d, value=%d\n",
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_EXIT:
		pr_err("%s GF_IOC_EXIT\n", __func__);
		break;

	case GF_IOC_DISABLE_IRQ:
		pr_err("%s GF_IOC_DISABEL_IRQ\n", __func__);
		gf_disable_irq(gf_dev);
		break;

	case GF_IOC_ENABLE_IRQ:
		pr_err("%s GF_IOC_ENABLE_IRQ\n", __func__);
		gf_enable_irq(gf_dev);
		break;

	case GF_IOC_RESET:
		pr_err("%s GF_IOC_RESET\n", __func__);
	...
	case GF_IOC_ENABLE_SPI_CLK:
		pr_err("%s GF_IOC_ENABLE_SPI_CLK\n", __func__);
#ifdef AP_CONTROL_CLK
		gfspi_ioctl_clk_enable(gf_dev);
#else
		pr_err("doesn't support control clock!\n");
#endif
		break;

	case GF_IOC_DISABLE_SPI_CLK:
		pr_err("%s GF_IOC_DISABLE_SPI_CLK\n", __func__);
#ifdef AP_CONTROL_CLK
	...
	case GF_IOC_ENTER_SLEEP_MODE:
		pr_err("%s GF_IOC_ENTER_SLEEP_MODE\n", __func__);
		break;

	case GF_IOC_GET_FW_INFO:
		pr_err("%s GF_IOC_GET_FW_INFO\n", __func__);
		break;

	case GF_IOC_REMOVE:
		pr_err("%s GF_IOC_REMOVE\n", __func__);
		irq_cleanup(gf_dev);
		gf_cleanup(gf_dev);
		break;

	case GF_IOC_CHIP_INFO:
		pr_err("%s GF_IOC_CHIP_INFO\n", __func__);
		if (copy_from_user(&info, (void __user *)arg, sizeof(struct gf_ioc_chip_info))) {
			retval = -EFAULT;
			break;
		}
		pr_err("gf vendor_id : 0x%x\n", info.vendor_id);
		pr_err("gf mode : 0x%x\n", info.mode);
		pr_err("gf operation: 0x%x\n", info.operation);
		break;
...
#endif
{
	...
	rc = gf_power_init(gf_dev);
	pr_err("gf  power init= %d\n", rc);
	if (status == 0) {
