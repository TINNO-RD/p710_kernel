...
static int sde_backlight_device_update_hbm_status(struct backlight_device *bd)
{
	int hbm_enable;
	struct dsi_display *display;
	struct sde_connector *c_conn;
	...
	hbm_enable = bd->props.hbm_enable;
	current_brightness = bd->props.brightness;

	c_conn = bl_get_data(bd);
	display = (struct dsi_display *) c_conn->display;

	if (c_conn->ops.set_hbm) {
		rc = c_conn->ops.set_hbm(&c_conn->base, display, hbm_enable, current_brightness);
	}
...
#ifdef JOURNEY_FEATURE_HAL_HBM
	//add for update hbm status begin:
	.update_hbm_status = sde_backlight_device_update_hbm_status,
	//add for update hbm status end
#endif
