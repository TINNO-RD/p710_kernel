		...
		struct sde_encoder_kickoff_params *params)
{
	...
	for (i = 0; i < sde_enc->num_phys_encs; i++) {
		...
		if (phys) {
#ifdef JOURNEY_FEATURE_HAL_HBM
            SDE_ATRACE_BEGIN("phys_prepare_for_kickoff");
            if(pending_vblank) {
                pending_vblank = 0;
                SDE_ATRACE_BEGIN("phys_wait_for_vblank");
                if (phys->ops.wait_for_vblank) {
                    rc = phys->ops.wait_for_vblank(phys);
                    if(rc) {
                        pr_warn("wait_for_vblank failed with rc %d" , rc);
                    }
                }
                SDE_ATRACE_END("phys_wait_for_vblank");
            }
#endif
...
#ifdef JOURNEY_FEATURE_HAL_HBM
            SDE_ATRACE_END("phys_prepare_for_kickoff");
#endif
		}
	}
