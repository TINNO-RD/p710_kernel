...
extern const bool HBM_ON_DRAWING;
#endif
		...
		struct drm_crtc *crtc)
{
    ...
    if (HBM_ON_DRAWING) {
	    ...
	    SDE_ATRACE_BEGIN("sde_conn_update_hbm");
        ...
        drm_for_each_connector_iter(drm_conn, &conn_iter) {
            ...
            if (sde_conn->ops.update_hbm) {
                sde_conn->ops.update_hbm(sde_conn->display,-1);
            }
        }
        drm_connector_list_iter_end(&conn_iter);
        SDE_ATRACE_END("sde_conn_update_hbm");
    }
#endif
		...
		struct sde_kms *sde_kms)
{
	static const struct sde_connector_ops dsi_ops = {
		...
		.set_hbm = dsi_display_req_hbm,
		.update_hbm = dsi_display_update_hbm,
