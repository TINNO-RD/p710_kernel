...
struct sde_connector_ops {
	...
	/**
	 * set_hbm - enable/disable hbm
	 * @connector: Pointer to drm connector structure
	 * @display: Pointer to private display structure
	 * @hbm_enable: 1: enable, 0: disable(set the user brightness which before enable hbm)
	 */
	int (*set_hbm)(struct drm_connector *connector,
			void *display, int hbm_enable, int current_brightness);
	/**
	 * update_hbm - update hbm
	 * @display: Pointer to private display structure
	 * @allow_hbm: 1: allow enable, 0: allow disable
	 */
	void (*update_hbm)(void *display, int allow_hbm);
