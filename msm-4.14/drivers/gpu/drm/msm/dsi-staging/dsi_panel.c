...
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
    JOURNEY_ATRACE_INT("dsi_cmd_count",count);
	JOURNEY_ATRACE_INT("dsi_cmd_type",type);
#endif
...
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
			JOURNEY_ATRACE_BEGIN("post_wait_ms");
#endif			
			usleep_range(cmds->post_wait_ms*1000,
					((cmds->post_wait_ms*1000)+10));
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
			JOURNEY_ATRACE_END("post_wait_ms");
#endif
		}
		cmds++;
	}
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
    JOURNEY_ATRACE_INT("dsi_cmd_count",0);
	JOURNEY_ATRACE_INT("dsi_cmd_type",0);
#endif
error:
	return rc;
}
...
int dsi_panel_set_nor1(struct mipi_dsi_device *dsi)
{
    int rc = 0;
    struct dsi_panel *panel;

    panel = container_of(dsi, struct dsi_panel, mipi_device);

    rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_NOR1);

    if (rc) {
	pr_err("[%s] failed to send DSI_CMD_SET_NOR1 cmds, rc=%d\n",
                panel->name, rc);
    }
...
int dsi_panel_set_nor2(struct mipi_dsi_device *dsi)
{
    int rc = 0;
    struct dsi_panel *panel;
    ...
    rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_NOR2);

    if (rc) {
	pr_err("[%s] failed to send DSI_CMD_SET_NOR2 cmds, rc=%d\n",
		panel->name, rc);
    }
...
int dsi_panel_set_hbm_on(struct mipi_dsi_device *dsi)
{
    int rc = 0;
    struct dsi_panel *panel;
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
...
int dsi_panel_set_hbm_off(struct mipi_dsi_device *dsi)
{
    int rc = 0;
    struct dsi_panel *panel;
...
static int dsi_panel_update_hbm(struct dsi_panel *panel, int hbm_enable, int current_brightness)
{
	int rc = 0;
	struct mipi_dsi_device *dsi;
	...
	dsi = &panel->mipi_device;
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
    JOURNEY_ATRACE_BEGIN("update_hbm");
    JOURNEY_ATRACE_INT("hbm_enable",hbm_enable);
#endif

	rc = mipi_dsi_dcs_set_display_hbm(dsi, hbm_enable, current_brightness);
	if (rc < 0)
		pr_err("failed to update dcs hbm: %d\n", hbm_enable);

#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
   JOURNEY_ATRACE_END("update_hbm");
#endif
	return rc;
}
...
#ifdef JOURNEY_FEATURE_HAL_HBM
//add for update hbm status begin:
int dsi_panel_set_hbm(struct dsi_panel *panel, int hbm_enable, int current_brightness)
{
	int rc = 0;

	rc = dsi_panel_update_hbm(panel, hbm_enable, current_brightness);
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	//tinno add set nor1 and nor2 gamma end
	//tinno add set hbm on and off begin:
	"qcom,mdss-dsi-hbm-on-command",
	"qcom,mdss-dsi-hbm-off-command",
	//tinno add set hbm on and off end
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	//tinno add set nor1 and nor2 gamma begin:
	"qcom,mdss-dsi-nor1-command-state",
	"qcom,mdss-dsi-nor2-command-state",
	//tinno add set nor1 and nor2 gamma end
	//tinno add set hbm on and off begin:
