...
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
#include <trace/journey_trace.h>
#endif
...
#ifdef JOURNEY_FEATURE_HAL_HBM
//add for update hbm status begin:
/*
HBM_ON_DRAWING == true: dsi_display_update_hbm in sde_kms_wait_for_commit_done 
HBM_ON_DRAWING == false: dsi_display_update_hbm in dsi_conn_post_kickoff and dsi_conn_pre_kickoff
*/
const bool HBM_ON_DRAWING = false; 
int dsi_hbm_req = 0;
int saved_current_brightness = 0;
int delay_frame = 0;
int pending_vblank = 0;
int dsi_display_req_hbm(struct drm_connector *connector,
		void *display, int hbm_enable, int current_brightness)
{
    struct dsi_display *dsi_display = display;

	mutex_lock(&dsi_display->display_hbm_lock);
    if(dsi_hbm_req != hbm_enable) {
        dsi_hbm_req = hbm_enable;
        saved_current_brightness = current_brightness;
        if(!HBM_ON_DRAWING)
            pending_vblank = 1; // all hbm need wait vblank
    }
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
    JOURNEY_ATRACE_INT("dsi_hbm_req",dsi_hbm_req);
#endif
	mutex_unlock(&dsi_display->display_hbm_lock);    
    return 0;
}

void dsi_display_update_hbm(void *display, int allow_hbm)
{
    int hbm_changed = 0;
    static int dsi_hbm_enable = 0;
	...
	mutex_lock(&dsi_display->display_hbm_lock);
    if((allow_hbm == dsi_hbm_req) || (allow_hbm == -1 /* both allow */)) {
        if(dsi_hbm_enable != dsi_hbm_req) { // not set
            if(delay_frame == 0) { // request dealy
                dsi_hbm_enable = dsi_hbm_req;
                current_brightness = saved_current_brightness;
                hbm_changed = 1;
            } else {
                delay_frame --;
            }
    ...
    if(hbm_changed) {
        pr_info("dsi_display_update_hbm call dsi_display_set_hbm with %d %d",dsi_hbm_enable,current_brightness);
        dsi_display_set_hbm(dsi_display,dsi_hbm_enable,current_brightness);
    }
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
    JOURNEY_ATRACE_FUNC_END();
#endif
}

int dsi_display_set_hbm(void *display, int hbm_enable, int current_brightness)
{
	struct dsi_display *dsi_display = display;
	struct dsi_panel *panel;
	int rc = 0;
	...
	rc = dsi_panel_set_hbm(panel, hbm_enable, current_brightness);
	if (rc)
		pr_err("unable to set hbm\n");
...
#include <dev_info.h>
char panel_name_info[60] = {0};
static int get_lcd_name_info(char *buf, void *arg0)
{
	if (panel_name_info != NULL) {
...
#ifdef CONFIG_TINNO_PRODUCT_INFO
	if(display->panel->name != NULL) {
		strcpy(panel_name_info, display->panel->name);
		FULL_PRODUCT_DEVICE_CB(ID_LCD, get_lcd_name_info, NULL);
	} else {
		pr_err("[%s] failed to get panel name\n", __func__);
	}
#endif
...
#ifdef JOURNEY_FEATURE_HAL_HBM
	mutex_init(&display->display_hbm_lock);
#endif
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) {
		...
		if (rc) {
			...
			DSI_MODE_FLAG_POMS)){
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
		JOURNEY_ATRACE_BEGIN("dsi_panel_enable");
#endif
		rc = dsi_panel_enable(display->panel);
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
		JOURNEY_ATRACE_END("dsi_panel_enable");
#endif
		if (rc) {
