		...
		struct drm_atomic_state *old_state)
{
	...
	for_each_new_connector_in_state(old_state, connector,
		...
		if (splash || (connector->state->crtc &&
			...
			SDE_ATRACE_BEGIN("MSM_DRM_EARLY_EVENT_BLANK");
#endif
			msm_drm_notifier_call_chain(MSM_DRM_EARLY_EVENT_BLANK,
					    &notifier_data);
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
			SDE_ATRACE_END("MSM_DRM_EARLY_EVENT_BLANK");
#endif
		}
		/*
		 * Each encoder has at most one connector (since we always steal
...
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
			SDE_ATRACE_BEGIN("MSM_DRM_EVENT_BLANK");
#endif
			msm_drm_notifier_call_chain(MSM_DRM_EVENT_BLANK,
					    &notifier_data);
#ifdef JOURNEY_FEATURE_KERNEL_ATRACE
			SDE_ATRACE_END("MSM_DRM_EVENT_BLANK");
#endif
		}
	}
	SDE_ATRACE_END("msm_enable");
