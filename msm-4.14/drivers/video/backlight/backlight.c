...
static int backlight_device_set_hbm(struct backlight_device *bd, int hbm_enable)
{
	int rc = -ENXIO;

	mutex_lock(&bd->ops_lock);
	if (bd->ops) {
		if (hbm_enable != 0 && hbm_enable != 1 && hbm_enable != 2) {
			pr_err("Set HBM mode failed, invaild parameters !!!\n");
			rc = -EINVAL;
		} else {
			pr_debug("set hbm to %d\n", hbm_enable);
			bd->props.hbm_enable = hbm_enable;
			rc = backlight_update_hbm_status(bd);
		}
	}
	mutex_unlock(&bd->ops_lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rc = kstrtoint(buf, 0, &hbm_enable);
	if (rc)
		return rc;

	rc = backlight_device_set_hbm(bd, hbm_enable);
