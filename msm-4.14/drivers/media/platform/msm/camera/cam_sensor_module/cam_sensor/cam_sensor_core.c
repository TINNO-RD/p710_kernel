...
//start-- modify for compatible different module by tinno 
#define MAX_MODULE_NUM 4
#define MAX_MODULE_SENSOR_NAME_SIZE 20
enum match_id_type_t {
	MODULE_ID,
	HW_PIN_ID,
};
struct module_info_t {
	...
	uint16_t sensor_id;
	int  match_id;
	unsigned char module_sensor_name[MAX_MODULE_SENSOR_NAME_SIZE];
	uint16_t module_addr;
	uint16_t match_id_type;
};
static struct module_info_t module_info_list[]={
	{0,0x0f8d1,0x43,"tsp_s5kgm1st",0xa0,MODULE_ID},
	{0,0x0f8d2,0x06,"sunwin_s5kgm1st",0xa0,MODULE_ID},
	{0,0x3109,0x43,"tsp_s5k3p9",0xa6,MODULE_ID},
	{0,0x3109,0x06,"sunwin_s5k3p9",0xa6,MODULE_ID},
	{0,0x02,0x1,"hlt_gc02m1b",0,HW_PIN_ID},
	{0,0x03,0x0,"kmj_gc02m1b",0,HW_PIN_ID},
};

uint16_t  get_module_id(struct cam_sensor_ctrl_t *s_ctrl,int index)
{
	int rc = 0,i2c_freq_mode=0;
	uint32_t module_id = 0;
	struct cam_cmd_i2c_info i2c_info_s;
	uint16_t back_slave_addr = s_ctrl->sensordata->slave_info.sensor_slave_addr;
	i2c_freq_mode = s_ctrl->io_master_info.cci_client->i2c_freq_mode;

	
	i2c_info_s.slave_addr= module_info_list[index].module_addr;
	i2c_info_s.i2c_freq_mode =I2C_FAST_MODE;
	cam_sensor_update_i2c_info(&i2c_info_s,s_ctrl);

	rc = camera_io_dev_read(
		&(s_ctrl->io_master_info),
		1,
		&module_id,  CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
	if (rc<0)
		CAM_ERR(CAM_SENSOR, "read module id faild,sensor_id:0x%x",s_ctrl->sensordata->slave_info.sensor_id);

	i2c_info_s.slave_addr= back_slave_addr;
	i2c_info_s.i2c_freq_mode =i2c_freq_mode;
	cam_sensor_update_i2c_info(&i2c_info_s,s_ctrl);
	
	CAM_INFO(CAM_SENSOR,"get_module_id:0x%x \n",module_id);

	return module_id;
}

int  get_hw_pin_id(int gpio)
{
    int      rc =0;
    rc = gpio_direction_input(gpio);
    if (rc<0){
	 CAM_ERR(CAM_SENSOR, "gpio_direction_input %d failed",gpio);
	 return rc;
     }

    rc = gpio_get_value_cansleep(gpio);
    if (rc<0){
	 CAM_ERR(CAM_SENSOR, "gpio_get_value_cansleep %d failed",gpio);
	 return rc;
     }
     CAM_INFO(CAM_SENSOR, "get_hw_pin_id :%d",rc);
    return rc;
}
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if ( abs(slave_info->sensor_id-chipid)<=(MAX_MODULE_NUM/2) ){
		for (i=0;i<ARRAY_SIZE(module_info_list);i++){
			if (slave_info->sensor_id == module_info_list[i].sensor_id){
				if (module_info_list[i].match_id_type == MODULE_ID)
					match_id = get_module_id(s_ctrl,i);
				else if (module_info_list[i].match_id_type == HW_PIN_ID)
					match_id = get_hw_pin_id(52);
				if (match_id == module_info_list[i].match_id){
					module_info_list[i].found = 1;
					CAM_INFO(CAM_SENSOR, " found id 0x%x name:%s",
			 		module_info_list[i].sensor_id,module_info_list[i].module_sensor_name);
					break;
				}else {
					CAM_ERR(CAM_SENSOR, "module_id:0x%x no match in module_info_list.module_id:0x%x",
						match_id,  module_info_list[i].match_id);
					continue;
				}
			}	
		}
		if (i>=ARRAY_SIZE(module_info_list)){
			CAM_INFO(CAM_SENSOR, "chip id: %x slave_id: %x no found in module_info_list",
				chipid, slave_info->sensor_id);
			return -ENODEV;
		}
	}
	...
	if (s_ctrl->soc_info.index == 0) {
		if (module_info_list[i].found){
			FULL_PRODUCT_DEVICE_INFO_CAMERA(s_ctrl->soc_info.index,s_ctrl->soc_info.index,module_info_list[i].module_sensor_name,8000,6000);
			module_info_list[i].found=0;
		}else{
			FULL_PRODUCT_DEVICE_INFO_CAMERA(s_ctrl->soc_info.index,s_ctrl->soc_info.index,"s5kgm1st",8000,6000);
		}
	}else if(s_ctrl->soc_info.index == 1) {
		if (module_info_list[i].found){
		    FULL_PRODUCT_DEVICE_INFO_CAMERA(s_ctrl->soc_info.index,2,module_info_list[i].module_sensor_name,1600,1200);
		    module_info_list[i].found=0;
		} else {
		    FULL_PRODUCT_DEVICE_INFO_CAMERA(s_ctrl->soc_info.index,2,"gc02m1b",1600,1200);
		}
	}else if(s_ctrl->soc_info.index == 2) {		
		if (module_info_list[i].found){
			FULL_PRODUCT_DEVICE_INFO_CAMERA(s_ctrl->soc_info.index,1,module_info_list[i].module_sensor_name,4640,3488);
			module_info_list[i].found=0;
		}else {
			FULL_PRODUCT_DEVICE_INFO_CAMERA(s_ctrl->soc_info.index,1,"s5k3p9",4640,3488);
		}
	}
#endif
	return rc;
}
//end-- modify for compatible different module by tinno 
