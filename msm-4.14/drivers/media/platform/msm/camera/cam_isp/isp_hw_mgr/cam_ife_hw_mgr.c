	...
	if (cfg->num_hw_update_entries > 0) {
		...
		if (cfg->init_packet) {
			...
			if (rc <= 0) {
				dump_stack();
				CAM_ERR(CAM_ISP,
					"config done completion timeout for req_id=%llu rc=%d ctx_index %d try_time:%d",
					cfg->request_id, rc, ctx->ctx_index,try_time);
				if (rc == 0 )
					rc = -ETIMEDOUT;
			} else {
				rc = 0;
