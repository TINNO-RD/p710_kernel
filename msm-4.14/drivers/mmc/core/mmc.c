...
static ssize_t mmc_chipinfo_show (struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if(si.totalram > 1572864 )
        ...
        strcpy(ramsize , "2G");
    else if(si.totalram > 131072)
        strcpy(ramsize , "1G");
    else
        strcpy(ramsize , "512M");
    
    printk("====emmc=size=%X\n",card->ext_csd.sectors); // x512/1024/1024
    if(card->ext_csd.sectors > 268435456)  		
	strcpy(romsize , "256G");
    else if(card->ext_csd.sectors > 134217728)   
        strcpy(romsize , "128G");
    else if(card->ext_csd.sectors > 67108864)
        strcpy(romsize , "64G");
    else if(card->ext_csd.sectors > 33554432)
        strcpy(romsize , "32G");
    else if(card->ext_csd.sectors > 16777216)
        strcpy(romsize , "16G");
    else if(card->ext_csd.sectors > 8388608)
        strcpy(romsize , "8G");
    else
        strcpy(romsize , "4G");
    ...
    if(strncasecmp((const char *)tempID, "90", 2) == 0)           // 90014A is OEMid for Hynix 
        strcpy(vendorName , "Hynix");
    else if(strncasecmp((const char *)tempID, "15", 2) == 0)      // 150100 is OEMid for Samsung 
        strcpy(vendorName , "Samsung");
    else if(strncasecmp((const char *)tempID, "45", 2) == 0)      // 450100 is OEMid for Sandisk 
        strcpy(vendorName , "Sandisk");
    else if(strncasecmp((const char *)tempID, "70", 2) == 0)      // 700100 is OEMid for Kingston 
...
static ssize_t mmc_emmcinfo_show (struct device *dev, struct device_attribute *attr, char *buf)
{
    struct mmc_card *card = mmc_dev_to_card(dev);
    char tempID[64] = "";
    memset(tempID, 0, sizeof(tempID));
    sprintf(tempID,"%02x%s%02x%02x%02x%02x%02x%01x%01x",card->cid.manfid,
													card->cid.prod_name,
													card->cid.prv,
													...
													card->ext_csd.device_life_time_est_typ_b,
													card->ext_csd.pre_eol_info,
													card->cid.month,card->cid.year-2013);
   
    return sprintf(buf,"%s", tempID);
    printk("Exit in show_emmcinfo_value\n");
    return sprintf(buf,"%s", tempID);
}

static DEVICE_ATTR(emmcinfo, S_IRUGO, mmc_emmcinfo_show, NULL); 
//odm add  chipinfo END
...
static struct attribute *mmc_std_attrs[] = {
	...
	&dev_attr_chipinfo.attr,  //odm add  chipinfo
	&dev_attr_emmcinfo.attr,  //odm add  chipinfo
	NULL,
};
