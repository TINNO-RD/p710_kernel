...
static int qpnp_tri_led_set(struct qpnp_led_dev *led)
{
	...
	dev_err(led->chip->dev, "PWM settings for %s led: period = %lluns, duty = %lluns,brightness = %d,blink=%d\n",
				led->cdev.name, period_ns, duty_ns,brightness,led->led_setting.blink);
		...
		enum led_brightness brightness)
{
	...
	dev_err(led->chip->dev, "Set led for %s, brightness=%d\n",
