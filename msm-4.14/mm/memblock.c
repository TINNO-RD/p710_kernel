...
#define PSTORE_BUF_ADDR   0x9df00000
#define PSTORE_BUF_SIZE   1024*1024
static ssize_t pstore_debug_read(struct file *file, char __user *ubuf,
                                    size_t count, loff_t *ppos)
{
    char *buf;
    const void *pstore = phys_to_virt(PSTORE_BUF_ADDR);
    int ret;
    unsigned int buf_size;

    buf = kzalloc(sizeof(char) * PSTORE_BUF_SIZE, GFP_KERNEL);
    if (!buf)
        return -ENOMEM;

    buf_size = ksize(buf);
    memcpy(buf,pstore,buf_size);

    ret = simple_read_from_buffer(ubuf, count, ppos, buf, buf_size);

    kfree(buf);
    return ret;
}

static const struct file_operations pstore_debug_fops = {
    .read = pstore_debug_read,
};
#endif
...
#endif
#ifdef JOURNEY_DEBUG_ENHANCED
    debugfs_create_file("pstore", S_IRUGO, root, NULL, &pstore_debug_fops);
#endif
	return 0;
}
