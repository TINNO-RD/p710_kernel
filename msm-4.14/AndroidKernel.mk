...
ifeq ($(JOURNEY_BUILD_SCRIPT),yes)
    ...
    ifeq ($(JOURNEY_FEATURE_FACTORY_VERSION_TPLCD_TEST), yes)
    KERNEL_JOURNEY_CFLAGS += -DJOURNEY_FEATURE_FACTORY_VERSION_TPLCD_TEST
    endif

    ifeq ($(JOURNEY_FEATURE_MANUAL_RAMDUMP), yes)
    KERNEL_JOURNEY_CFLAGS += -DJOURNEY_FEATURE_MANUAL_RAMDUMP
    endif

    ifeq ($(JOURNEY_FEATURE_FORCE_WARM_RESET), yes)
    KERNEL_JOURNEY_CFLAGS += -DJOURNEY_FEATURE_FORCE_WARM_RESET
    endif

    ifeq ($(JOURNEY_FEATURE_HAL_HBM), yes)
    KERNEL_JOURNEY_CFLAGS += -DJOURNEY_FEATURE_HAL_HBM
    endif

    ifeq ($(JOURNEY_FEATURE_KERNEL_ATRACE), yes)
    KERNEL_JOURNEY_CFLAGS += -DJOURNEY_FEATURE_KERNEL_ATRACE
    endif

    KERNEL_MAKE_ENV += KERNEL_JOURNEY_CFLAGS="$(KERNEL_JOURNEY_CFLAGS)"
    $(warning KERNEL_MAKE_ENV $(KERNEL_MAKE_ENV))
else
...
# fenghui.zou modified for CONFIG_DYNAMIC_DEBUG open on debug version
# Reset variable
...
$(INSTALLED_DTBIMAGE_TARGET): $(TARGET_PREBUILT_INT_KERNEL) $(INSTALLED_KERNEL_TARGET) $(RTIC_DTB)
	$(hide) if [ -d "$(KERNEL_OUT)/arch/$(KERNEL_ARCH)/boot/dts/tinno/" ]; then \
			cat $(KERNEL_OUT)/arch/$(KERNEL_ARCH)/boot/dts/tinno/$(TARGET_PRODUCT)/*.dtb $(RTIC_DTB) > $@; \
		else \
